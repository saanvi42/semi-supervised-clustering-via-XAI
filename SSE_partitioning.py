...
class FlatSSE():
    def __init__(self, A, A_con, num_cluster=None, mustlink_first = None):
        ...
        # record merge history for explainability: list of dicts
        # each entry: {step, comm1, comm2, deltaH, new_members}
        self.merge_history = []
        self._merge_step = 0
    ...
    def merge(self):
        ...
        while len(merge_queue) > 0:
            ...
            new_members = comm1[0].union(comm2[0])
            new_comm = (new_members, comm1[1]+comm2[1], comm1[2]+comm2[2]-2*self.pair_cuts[frozenset([commID1,commID2])],
                        g_conx, comm1[4]+comm2[4]-deltaH)
            self.communities[commID1] = new_comm
            self.communities.pop(commID2)

            # record merge step for explainability
            try:
            ...
            self.merge_history.append({
                ...
                'comm2': int(commID2),
                'deltaH': float(deltaH),
                'mustlink_flag': int(pair_mustlink),
